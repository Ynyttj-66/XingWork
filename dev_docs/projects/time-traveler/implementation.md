...
在 XingWork 的对话界面中，用户可以通过右键菜单撤回自己发送的最后一条消息（及其对应的工具调用和 AI 回复），使对话状态回退到上一轮之前。
...
设计文档原本提议通过 `agent.get_state()` / `agent.update_state()` 操作状态。在 XingWork 中，Agent 图每次在 `_run_agent_turn` 中构建，未持久化在 session 上。
